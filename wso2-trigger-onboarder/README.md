...
integration **trigger** to the WSO2 Integrator. It drives the changes across both repos, writes and runs
the language-server tests, and opens two PRs.
...
- Branches off a user-chosen base branch (local or upstream), pushes to your fork, and opens two PRs
...
- Local clones (or forks) of `ballerina-language-server` and `vscode-extensions`,
...
- *"Wire up a new Kafka-style event trigger across both repos"*

The agent asks a few questions first (the Ballerina module, a form preview, and icons), then implements the
changes, runs the LS tests, and — after confirming with you — opens the two PRs.
